...
datos <- read_xlsx("C:/.../dolarblue.xlsx")
datosts <- ts(datos$blue, start = c(2017,1), frequency = 12)
datostsNA <- datosts
...
ggplot(datos, aes(x = Fecha, y = blue)) +
  geom_line(linewidth = 1) +
  geom_point(color = "#1874CD", size = 2.5) +
  labs(
    title = "Gráfico 4.1.1: Precio del dólar blue desde el 2017 al 2024",
    x = "Año",
...
ggAcf(datosts, plot=TRUE,  lag.max=120) +
  labs(
    title = "Gráfico 4.1.2: Función de autocorrelación muestral",
    x = "Rezago",
    y = "FACM"
  ) +
  theme_minimal() + 
  ...
  scale_y_continuous(
    limits = c(-0.5,1),
    breaks = c(-0.50, -0.25, 0.00, 0.25, 0.50, 0.75, 1.00)
  )

ggPacf(datosts, plot=TRUE, lag.max=120) +
  labs(
    title = "Gráfico 4.1.3: Función de autocorrelación parcial muestral",
    x = "Rezago",
    y = "FACPM"
  ) +
  theme_minimal() + 
...
ggseasonplot(datosts, year.labels = TRUE, yer.labels.size = 15, xlab="Mes", ylab="Precio del Dólar Blue ($)") +
   ggtitle("Gráfico 4.1.4: Comportamiento anual del precio del dólar blue") +
   geom_line(size = 2) +
   theme_minimal() +
   theme(
...
ggsubseriesplot(datosts) + 
  ggtitle("Gráfico 4.1.5: Comportamiento mensual del precio del dólar blue") +
  xlab("Mes") + 
  ylab("Precio dólar blue ($)") +
  geom_line(size = 1) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 26, face = "bold"),
...
probabilidad_falta_mnar <- pmin(1, abs(datosts - mean(datosts)) / sd(datosts))
...
plot(dolar_ts_mnar, main = "Serie con valores faltantes MNAR", ylab = "Precio", xlab = "Año")
...
tabla <- read_excel("C:/.../tablas.xlsx", sheet = "Hoja1")
